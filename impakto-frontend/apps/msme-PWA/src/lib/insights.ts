import { ledgerService, TransactionResponse } from './ledger';
import { scoringService, TrustScoreData } from './scoring';

// ─── Types ────────────────────────────────────────────────────────────────────

export type InsightType = 'loan' | 'best_seller' | 'restock' | 'profit';

export interface Insight {
  type: InsightType;
  title: string;
  body: string;
  /** Only present for the 'loan' card */
  cta?: string;
}

export interface InsightsResult {
  insights: Insight[];
  /** True when there are fewer transactions than the minimum needed */
...
function getBestSellerInsight(txns: TransactionResponse[]): Insight {
 ...
 */
function getRestockInsight(txns: TransactionResponse[]): Insight {
  const incomes = txns.filter(t => t.transaction_type === 'INCOME');
  ...
  const avgDaily =
    Object.values(dailyMap).reduce((s, v) => s + v, 0) / days.length;
...
function getProfitInsight(txns: TransactionResponse[]): Insight {
...
function getLoanInsight(score: TrustScoreData, income: number): Insight | null {
  if (score.growth_trend < LOAN_GROWTH_THRESHOLD) return null;

  const growthPct = Math.round(
    ((score.growth_trend - 50) / 100) * 100 // engine maps growth to 50–150 pts
  );

  const suggestedAmount =
    income > 0 ? Math.round((income * 0.5) / 1000) * 1000 : 10000;
...
export const insightsService = {
  async getInsights(): Promise<InsightsResult> {
    ...
    if (loanCard) insights.push(loanCard);
